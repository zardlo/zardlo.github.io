Move={
	bandall:function(){  /*绑定移动特效*/
		$(".map_alert").delegate(".tip","mousedown",function(e) {
				if(e.target.className!="del"){
					$(".tips").stop().animate({left:'-'+$(".tips").width()},500)
				}
			    newtip="";
				if($(this).parent(".tips").length!=0){
...
$(function(){
	Move.bandall();/*绑定*/
	$(".map_alert").delegate(".del","click",function(){
			var obj=$(this).parent().clone().removeAttr("style").css({"visibility":"hidden"})
			obj.appendTo(".tips")
			var t=obj.offset().top-$(".map_alert").offset().top
			var l=obj.offset().left-$(".map_alert").offset().left
	...
	$(".savebtn").click(function(){
		var id='',left='',top='';
		/*$(".pic>.tip").each(function(i){
			
			id+=$(this).attr("rel")+'|';
			top+=$(this).css("top")+'|';
			left+=$(this).css("left")+'|';
		})*/
		$(".pic>.tip2").each(function(i){
			
			id+=$(this).attr("rel")+'|';
			top+=$(this).css("top")+'|';
			left+=$(this).css("left")+'|';
		})
		data="id="+id+"&top="+top+"&left="+left+'&dev_id='+index;
		$.ajax({
			type:"GET",
			url:"dev_add/leak_data_form.php?"+data,
			dateType:"html",
			cache:false,
			data:data,
